...
function button_el_det() {
	...
	*/
	function makeOrderInterface(state, event) {
		...
		var itemCurrentPrice = parseInt($(".pcard-store-prices").not(".non").find('.pcard-store__price-current').text().replace(/\s+/g, ''));
		...
		if(state === 'init') {
            ...
            promoCode = "";
            bonusCard = "";
            $('#pcard-promo-code-form, #pcard-bonus-card-form').off("submit");
            $('#pcard-promo-code-form').find("input[type=text]").val("").attr("disabled",false);
            $('#pcard-bonus-card-form').find("input[type=text]").val("");
            $('.pcard-order-promo-total__before').removeClass("pcard-order-promo-total__before--yes");
            $('#pcard-promo-code-button, #pcard-bonus-card-button').removeClass("pcard-order-promo-submit--valid").addClass("pcard-order-promo-submit--invalid");
            $('.pcard-order-promo-warning').removeClass("pcard-order-promo-warning--invalid").addClass("pcard-order-promo-warning--valid");
			...
			// отображение цены в попапе
			$('.pcard-order-promo-total__after').find('b').text(itemCurrentPrice);
			...
			$('#pcard-promo-code-form').on('submit', function (event) {
                ...
                $.getJSON(ajax_path_bron + "check_coupon.php", {coupon:promoCode, id:offerId}, function (data) {
                    ...
                    /*
                       ...
                       itemCurrentPrice и curPrice  сиинонимы
                    */
					...
					// для промокода
					// УБРАТЬ
					//if(promoCode === '123') data.price = 0.5 * itemCurrentPrice;
					console.log('data.price = ' + data.price);
					console.log('itemCurrentPrice = ' + itemCurrentPrice);
					// УБРАТЬ


					if(data.price === 0) {
						$('.pcard-order-promo-total__before').removeClass('pcard-order-promo-total__before--yes');
						$('.pcard-promo-code-form').find('.pcard-order-promo-warning').text('купон не найден');
					}
					else if(data.price === itemCurrentPrice) {
						$('.pcard-order-promo-total__before').removeClass('pcard-order-promo-total__before--yes');
						$('.pcard-promo-code-form').find('.pcard-order-promo-warning').text('купон не для этого товара');
					}
					else if(data.price < itemCurrentPrice) {
						promoCodeValid = true;
						// $('.pcard-promo-code-form').find('.pcard-order-promo-warning').text();
						$('.pcard-order-promo-total__before').find('b').text(itemCurrentPrice);
						$('.pcard-order-promo-total__after').find('b').text(data.price);
						$('.pcard-order-promo-total__before').addClass('pcard-order-promo-total__before--yes');
					}
...
console.log('smscheckcode');
	...
	$(document).on('click', function (event) {
		...
		if($(event.target).hasClass('pcard-store__reserve-button'))
		{
			...
			makeOrderInterface('cancel', event);
			...
			makeOrderInterface('getshop', event);
			...
			if ($.cookie('GOLD585_ORDER_PHONE') != undefined) {
				...
				makeOrderInterface('smscheckcode', event);
			} else {
				makeOrderInterface('smsstart', event);
			}
		}
